function getEligibleDates(digit) {
  const today = new Date()
  today.setHours(0, 0, 0, 0)

  const isEven = digit % 2 === 0
  const todayEligible = isEven ? today.getDate() % 2 === 0 : today.getDate() % 2 === 1
  ...
  while (upcomingDays.length < 4) {
    ...
    if (isEligible) {
      upcomingDays.push({
        ...
        dateStr: searchDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
      })
    }
...
export default function FuelStatus({ digit, onReset }) {
  const { todayEligible, todayEligibleInfo, nextThree } = getEligibleDates(digit)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
      ...
      {/* Today's Status */}
      <div className={`p-6 rounded-xl mb-6 ${todayEligible ? 'bg-green-50 border-2 border-green-200' : 'bg-red-50 border-2 border-red-200'}`}>
        <h3 className={`text-xl font-bold mb-2 ${todayEligible ? 'text-green-700' : 'text-red-700'}`}>
          {todayEligible ? '✅ You CAN fuel today!' : '❌ You CANNOT fuel today'}
        </h3>
        <p className={`text-sm ${todayEligible ? 'text-green-600' : 'text-red-600'}`}>
          {todayEligibleInfo ? `Today: ${todayEligibleInfo.dayName}, ${todayEligibleInfo.dateStr}` : 'Check next eligible date below'}
        </p>
      </div>
